...
module KitchenHooks
  class App < Sinatra::Application
    ...
    def self.data_bag_from_file(data_bag, items, knives)
      ...
      begin
        items.each do |item|
          next unless item.split('.').last.among?('json')
    ...
    def self.role_from_file(roles, knives)
      ...
      begin
        roles.each do |role|
          next unless role.split('.').last.among?('json', 'rb')
    ...
    def self.environment_from_file(environments, knives)
      ...
      begin
        environments.each do |environment|
          next unless environment.split('.').last.among?('json', 'rb')
    ...
    def self.berks_install(berksfile, knife = nil)
      ...
      begin
        ...
        out = `#{cmd}`
        raise out unless $CHILD_STATUS.exitstatus.zero?
      rescue
    ...
    def self.notification(entry)
      ...
      when 'synced', 'unsynced'
        if event.is_a? String
          ...
          deletions = '' unless num_deletions.positive?
      ...
      when 'kitchen upload'
        %| <i>#{author(event)}</i> updated <a href="#{gitlab_url(event)}">the Kitchen</a> |
      when 'cookbook upload'
        %| <i>#{author(event)}</i> released <a href="#{gitlab_tag_url(event)}">#{tag_name(event)}</a> of <a href="#{gitlab_url(event)}">#{cookbook_name(event)}</a> |
      when 'constraint application'
        %| <i>#{author(event)}</i> constrained <a href="#{gitlab_tag_url(event)}">#{tag_name(event)}</a> with <a href="#{gitlab_url(event)}">#{cookbook_name(event)}</a> #{version_link event} |
      when 'release'
        " Kitchen Hooks <b>v#{event}</b> released! "
      when 'upload from files'
        %| <i>#{author(event)}</i> uploaded <a href="#{gitlab_url(event)}">files</a> to chef |
      else
        raise entry.inspect
      end.strip
    end
    ...
    def self.files_pushed(event)
      ...
      event.fetch('commits').each do |commit|
        files << commit.select { |k, _v| k =~ /(added|modified)/ }.values.flatten
      end
      files.flatten.uniq
    end
    ...
    def self.version_url(event)
      ...
      url = git_daemon_style_url(event).sub(/^git/, 'http').sub(/\.git$/, '')
      "#{url}/commits/#{event['version']}"
    end

    def self.version_link(event)
      return unless event['version']
      url = version_url(event)
      'at <a href="%s">%s</a>' % [url, event['version']]
    end
  end
end
